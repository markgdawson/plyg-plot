...
class TorqueFile:
    ...
    def getMeanCpOverLastNRevs(self,nRev,plot=False,xunits='revs'):
        ...
        meancp,tStepStart,tStepEnd,lines = self.getMeanCpOverRevs(iRev,nRev,plot=plot)
    ...
    def drawTorqueVector(self,iStep,iPatch,xunits='revs',scalefactor=3000,linestyle='-',color=False):
        ...
        plt.arrow(Xstart[0],Xstart[1],Xend[0]-Xstart[0],Xend[1]-Xstart[1], head_width=0.1, head_length=0.2, fc=color, ec=color, linestyle=linestyle, linewidth=1)
    ...
    def _plotTransientOverRange(self,transient,tSteps=None,xunits='revs',ylabel=None,color=None):
    ...
    def getMeanTorqueSlidingWindow(self, n_revs_window=1, patches=None):
        ...
        meanTorquePrevRev = np.array([ np.mean(totalTorquePerTimeStep[iTS:iTS+nSteps]) for iTS in range(0,self.nTS-nSteps) ])
    ...
    def plotTransientCp(self, xunits='time'):
        CpPerTimeStep = self.getCpPerTimeStep()
        
        return self._plotTransientOverRange(CpPerTimeStep,xunits='revs',ylabel='Cp')
